...
## 概述
Auto Evolution 为 Agent 增加一套安全、可控、可持续优化的反馈闭环。它会静默捕捉用户的纠错反馈，把重复出现的问题沉淀成规则候选，对持续低分的 Skill 提出优化建议，并在现有 Skill 无覆盖时提议创建新 Skill。它的原则很明确：可以自动观察，但不能自动乱改，所有正式变更都必须经过用户确认。
